...
class PlaceOrder extends Component{
    ...
    addressSubmitHandler = (e) => {
        ...
        if(this.state.address.fullName === ''){
            this.setError(true, 'Enter Full Name'); return;
        }
        if(this.state.address.mobileNumber === ''){
            this.setError(true, 'Enter Mobile');
            return;
        }
        const mobilePattern = new RegExp(/[0-9]{10}/);
        if(!mobilePattern.test(this.state.address.mobileNumber)){
            this.setError(true, 'Invalid Phone Number'); return;
        }
        

        const address = {
            userId: JSON.parse(window.localStorage.getItem('auth')).user.userId,
       ...
       fetch(`${base_url}/user/new-address`, {
            headers: {
                ...
                'auth-token': JSON.parse(window.localStorage.getItem('auth')).token
            },
            method: 'POST',
